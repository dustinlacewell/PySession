            ...
            # Built-In Commands
                ...
                if command == "move":
                    if len(args) == 0:
                        ...
                        self.join(self.help_channel)
                if command == "admins":
                    self.msg(self.help_channel, self.engine.admins())
...
class IRCBotFactory(protocol.ReconnectingClientFactory):
    ...
    def buildProtocol(self, addr):
        conf = self._engine.ircconf
        p = self.protocol()
        p.factory = self
        p.engine = self._engine
        p.nickname = conf.nickname
        p.password = conf.nickpass
        p.help_channel = conf.helpchannel
        p.channel_pass = conf.channelpass
        self.p = p
        return self.p
...
class IRCService(internet.TCPClient):
    ...
    def __init__(self, eng):
        ...
        network = eng.ircconf.network
        ports = [int(port) for port in eng.ircconf.port.split()]
        self.engine = eng
